...
public class BillServiceImpl implements BillService {
    ...
    private BigDecimal computeProductPrice(BaseCustomer customer, BigDecimal totalOrderPrice, BaseProduct product, Integer count) {
        if (isAvailableAgeForProductPay(customer, product)) {
            String msg = "[" + new Date() + "]. Failed add product because age less than product limit age";
            LOG.info(msg);
        } else {
            ...
            String format = String.format("[%s] take product: %s, %d, customer: %s", new Date(), product, count, customer);
            LOG.info(format);
        }
        return totalOrderPrice;
    }
    ...
    private BigDecimal tryApplyDiscount(BaseCustomer customer, BaseProduct product, BigDecimal productTotalPrice) {
        if (customer instanceof Retired) {
            ...
            String format = String.format("[%s] apply bonus: 0.9, product%s ,  customer: %s", new Date(), product, customer);
            LOG.info(format);
        }
